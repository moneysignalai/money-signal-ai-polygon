...
async def run_options_whales() -> None:
        ...
        try:
            ...
            for c in contracts:
                if c.underlying_price is not None and c.underlying_price < OPTIONS_MIN_UNDERLYING_PRICE:
                    ...
                    debug_filter_reason(BOT_NAME, c.contract, "whale_underlying_price_too_low")
                    continue
                if c.dte is not None and c.dte > WHALES_MAX_DTE:
                    reason_counts["dte"] = reason_counts.get("dte", 0) + 1
                    debug_filter_reason(BOT_NAME, c.contract, "whale_dte_too_long")
                    continue
                if c.notional is None or c.size is None:
                    reason_counts["missing_prices"] = reason_counts.get("missing_prices", 0) + 1
                    debug_filter_reason(BOT_NAME, c.contract, "whale_missing_price_size")
                    continue
                if c.notional < WHALES_MIN_NOTIONAL or c.size < WHALES_MIN_SIZE:
                    ...
                    debug_filter_reason(BOT_NAME, c.contract, "whale_notional_or_size_too_low")
                    continue
                ...
                alerts += 1
                send_alert("WHALE FLOW", symbol, c.underlying_price or 0.0, 0.0, extra=text)
    ...
    if DEBUG_FLOW_REASONS and matches == 0:
        print(f"[options_whales] No alerts. Filter breakdown: {reason_counts}")
